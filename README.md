...
```python
converter = SlackMarkdownConverter(encoding="utf-8")
```

### Escaping Slack Special Characters

Slack's mrkdwn spec requires `&`, `<` and `>` to be escaped in plain text. This is
disabled by default to keep output backward-compatible. Enable it with the
`escape_special_chars` flag:

```python
converter = SlackMarkdownConverter(escape_special_chars=True)
print(converter.convert("A & B < C > D"))  # Output: A &amp; B &lt; C &gt; D
```

Escaping is never applied inside code spans/blocks, to the leading blockquote
marker, or to links generated by the converter.
